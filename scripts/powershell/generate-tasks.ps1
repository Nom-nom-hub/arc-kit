...
if (-not (Test-Path $planFile)) {
    if ($Json) {
        ...
        Write-Error "Error: No implementation plan found in current branch or spec directories"
    }
    exit 1
}
...
if ($Json) {
    ...
    Write-Output "Created task chain: $tasksFile"
    Write-Output "Based on plan: $planFile"
