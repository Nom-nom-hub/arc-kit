...
<#
.SYNOPSIS
    Build Arc Kit template release archives for each supported AI assistant and script type.
...
.PARAMETER Agents
    ...
    Valid agents: claude, gemini, copilot, cursor-agent, qwen, opencode, windsurf, codex, kilocode, auggie, roo, codebuddy, amp, shai, q
...
function Rewrite-Paths {
    ...
    $Content = $Content -replace '(/?)\bmemory/', '.arc/memory/'
    $Content = $Content -replace '(/?)\bscripts/', '.arc/scripts/'
    $Content = $Content -replace '(/?)\btemplates/', '.arc/templates/'
    return $Content
}
...
function Generate-Commands {
    ...
    foreach ($template in $templates) {
        ...
        $outputFile = Join-Path $OutputDir "arckit.$name.$Extension"
...
function Generate-CopilotPrompts {
    ...
    $agentFiles = Get-ChildItem -Path "$AgentsDir/arckit.*.agent.md" -File -ErrorAction SilentlyContinue
...
function Build-Variant {
    ...
    $baseDir = Join-Path $GenReleasesDir "arc-kit-${Agent}-package-${Script}"
    Write-Host "Building $Agent ($Script) package..."
    New-Item -ItemType Directory -Path $baseDir -Force | Out-Null
    
    # Copy base structure but filter scripts by variant
    $arcDir = Join-Path $baseDir ".arc"
    New-Item -ItemType Directory -Path $arcDir -Force | Out-Null
    
    # Copy memory directory
    if (Test-Path "memory") {
        Copy-Item -Path "memory" -Destination $arcDir -Recurse -Force
        Write-Host "Copied memory -> .arc"
    }
    
    # Only copy the relevant script variant directory
    if (Test-Path "scripts") {
        $scriptsDestDir = Join-Path $arcDir "scripts"
    ...
    if (Test-Path "templates") {
        $templatesDestDir = Join-Path $arcDir "templates"
        ...
        Write-Host "Copied templates -> .arc/templates"
    }
    ...
    # Create zip archive
    $zipFile = Join-Path $GenReleasesDir "arc-kit-template-${Agent}-${Script}-${Version}.zip"
    Compress-Archive -Path "$baseDir/*" -DestinationPath $zipFile -Force
    Write-Host "Created $zipFile"
}

# Define all agents and scripts
$AllAgents = @('claude', 'gemini', 'copilot', 'cursor-agent', 'qwen', 'opencode', 'windsurf', 'codex', 'kilocode', 'auggie', 'roo', 'codebuddy', 'amp', 'shai', 'q')
$AllScripts = @('sh', 'ps')
...
Get-ChildItem -Path $GenReleasesDir -Filter "arc-kit-template-*-${Version}.zip" | ForEach-Object {
    Write-Host "  $($_.Name)"
}
